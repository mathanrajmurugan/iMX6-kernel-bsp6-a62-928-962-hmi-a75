...
static ssize_t wdog_trigger_status_read (struct class *cls, struct class_attribute *attr, char *buf) {
...
static ssize_t wdog_trigger_status_write (struct class *cls, struct class_attribute *attr, const char *buf, size_t count) {
	char *endp;
...
static ssize_t wdog_trigger_mode_read (struct class *cls, struct class_attribute *attr, char *buf) {
...
static ssize_t wdog_trigger_mode_write (struct class *cls, struct class_attribute *attr, const char *buf, size_t count) {
	char *endp;
...
static ssize_t wdog_trigger_refresh_time_read (struct class *cls, struct class_attribute *attr, char *buf) {

	return sprintf(buf, "%u msec\n", jiffies_to_msecs(wdt.wdt_refresh_time));
}


static ssize_t wdog_trigger_refresh_time_write (struct class *cls, struct class_attribute *attr, const char *buf, size_t count) {
	char *endp;
...
static struct class_attribute apx_attr[] ={
	__ATTR(status, 0644, wdog_trigger_status_read, wdog_trigger_status_write),
	__ATTR(trigger_mode, 0644, wdog_trigger_mode_read, wdog_trigger_mode_write),
	__ATTR(refresh_time, 0644, wdog_trigger_refresh_time_read, wdog_trigger_refresh_time_write),
	__ATTR(residual_time, 0660, wdog_trigger_residual_time_read, NULL),
	__ATTR(refresh, 0220, NULL, wdog_trigger_refresh_write),
	__ATTR_NULL
};
...
static struct class wdog_drv ={
		.name = "apx_wdog",
		.owner = THIS_MODULE,
		.class_attrs =(struct class_attribute *) &apx_attr,
};
...
static long apx_wdt_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
	...
	switch ( cmd ) {
		...
		case APX_WDT_SET_STATUS: {
			...
			if (copy_from_user (&status, (const void __user *)arg, sizeof (status))) {
				mutex_unlock (&wdt.ioctl_lock);
				return -EFAULT;
			}
		...
		case APX_WDT_GET_TRIGGER_MODE: {
			...
			if ( copy_to_user ((void __user *)arg, &trigger_mode, sizeof(int)) ) {
				mutex_unlock (&wdt.ioctl_lock);
				return -EFAULT;
			}
		...
		case APX_WDT_SET_TRIGGER_MODE: {
			...
			if (copy_from_user (&trigger_mode, (const void __user *)arg, sizeof (trigger_mode))) {
				mutex_unlock (&wdt.ioctl_lock);
				return -EFAULT;
			}
		...
		case APX_WDT_GET_REFRESH_TIME: {
			...
			if ( copy_to_user ((void __user *)arg, &refresh_time, sizeof(unsigned long)) ) {
				mutex_unlock (&wdt.ioctl_lock);
				return -EFAULT;
			}
		...
		case APX_WDT_SET_REFRESH_TIME: {
			...
			if (copy_from_user (&refresh_time, (const void __user *)arg, sizeof (refresh_time))) {
				mutex_unlock (&wdt.ioctl_lock);
				return -EFAULT;
			}
		...
		case APX_WDT_GET_RESIDUAL_TIME: {
			...
			if ( copy_to_user ((void __user *)arg, &residual_time, sizeof(unsigned long)) ) {
				mutex_unlock (&wdt.ioctl_lock);
				return -EFAULT;
			}
...
static int __init apx_wdt_class_init(void)
{
	...
	apx_wdt_class->class_attrs =(struct class_attribute *) &apx_attr;
	return 0;
}
